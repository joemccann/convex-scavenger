...
describe("isCriDataStale — market OPEN", () => {
  it("NOT stale when market open and mtime within TTL", () => {
    expect(isCriDataStale({ date: TODAY, market_open: true }, RECENT_MTIME, TODAY)).toBe(false);
  });

  it("stale when market open and mtime exceeds TTL", () => {
    expect(isCriDataStale({ date: TODAY, market_open: true }, STALE_MTIME, TODAY)).toBe(true);
  });
});
...
describe("isCriDataStale — market CLOSED (EOD data should never re-trigger scan)", () => {
  it("NOT stale when market closed + date is today, even with stale mtime", () => {
    expect(isCriDataStale({ date: TODAY, market_open: false }, STALE_MTIME, TODAY)).toBe(false);
  });

  it("NOT stale when market closed + date is today, recent mtime", () => {
    expect(isCriDataStale({ date: TODAY, market_open: false }, RECENT_MTIME, TODAY)).toBe(false);
  });
...
describe("isCriDataStale — date mismatch (always stale)", () => {
  it("stale when data date is yesterday regardless of market_open", () => {
    expect(isCriDataStale({ date: "2026-03-08", market_open: true }, RECENT_MTIME, TODAY)).toBe(true);
  });

  it("stale when data date is yesterday and market closed (pre-open)", () => {
    expect(isCriDataStale({ date: "2026-03-08", market_open: false }, RECENT_MTIME, TODAY)).toBe(true);
  });

  it("stale when data has no date field", () => {
    expect(isCriDataStale({ market_open: false }, STALE_MTIME, TODAY)).toBe(true);
  });
});
...
describe("isCriDataStale — market_open unknown/missing", () => {
  it("falls back to TTL check when market_open is undefined + date matches", () => {
    ...
    expect(isCriDataStale({ date: TODAY }, RECENT_MTIME, TODAY)).toBe(false);
    expect(isCriDataStale({ date: TODAY }, STALE_MTIME, TODAY)).toBe(true);
  });
});
