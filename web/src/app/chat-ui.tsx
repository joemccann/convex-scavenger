...
import { createConvexStreamFn } from "@/lib/convex-stream";

// Register custom message renderers
if (typeof registerCustomMessageRenderers === "function") {
...
Feel free to use these tools when needed to provide accurate and helpful responses.`,
		...
		// Route messages through the local /api/chat backend
		streamFn: createConvexStreamFn(),
				...
				<div class="flex items-center gap-2 px-4 py-2">
					...
					${
						currentTitle
							...
							: html`<span class="text-base font-semibold text-foreground">Convex Scavenger</span>`
					}
				</div>
...
async function initApp() {
	...
	// Seed a placeholder API key so AgentInterface's pre-send check passes.
	// The actual API call is routed through /api/chat (server-side), not direct to Anthropic.
	const existingKey = await providerKeys.get("anthropic");
	if (!existingKey) {
		await providerKeys.set("anthropic", "local-backend-proxy");
	}
