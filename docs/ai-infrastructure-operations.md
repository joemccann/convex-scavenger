...
Omitting both modes is equivalent to `--verify`. `--record` without `--database` uses the production store, unless `RADON_AI_CYCLE_DB_PATH` selects an isolated SQLite file. Production cycles emit bounded service-health heartbeats, including enabled-source failures and unchanged cycles. Verification and isolated SQLite cycles never write production health state. Missing credentials are visible unavailable states; HTTP failures and schema errors remain explicit. Failed source observations are never replaced by zero.

`--env-file PATH` reads only `OPENROUTER_API_KEY`, `ARTIFICIAL_ANALYSIS_API_KEY`, `SEC_USER_AGENT`, `EIA_API_KEY`, and `VAST_API_KEY`. Environment variables take precedence. The service's environment file supplies production values; credentials are never copied into source URLs, status reasons or logs. `SEC_USER_AGENT` must identify the application and a real contact email. Production EIA collection requires its own registered key; `DEMO_KEY` was used only for the isolated public-access probe.
